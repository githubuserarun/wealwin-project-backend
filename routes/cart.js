...
router.post('/add', verifyToken, async (req, res) => {
    ...
    const existCartItem = await cartCollection.findOne({ userId, productId:_id });
...
router.delete('/del/:id', verifyToken, async (req, res) => {
    ...
    try {
        const userId = req.authData.user.id
        //console.log(userId);
        const findCart = await cartCollection.findByIdAndDelete({ userId, _id: id });
        // console.log(findCart)
        if (!findCart) {

            res.json({ message: "cart Item not found", status: false })
        } else {
            res.json({ data: findCart, message: "cart Item successfully deleted", status: true })
        }
...
router.put('/update', async (req, res) => {
    ...
    try {
        ...
        if (operation === "inc") {
            ...
            return res.json({ status: false, message: "Invalid operation specified" });
        }
        ...
        return res.json({ data: findCart, message: "Cart item successfully updated", status: true });
